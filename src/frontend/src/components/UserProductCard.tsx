...
export function UserProductCard({ product, index }: UserProductCardProps) {
  const { addItem, items } = useCart();
  const inCart = items.some((i) => i.productId === product.id);
  const { currencyInfo } = useCurrency();

  // price is stored in kopecks (RUB base). Convert to USD, then to user currency.
  const RUB_RATE = getCurrencyInfo("RUB").rateToUsd; // 91.5
  const priceUsd = product.price / 100 / RUB_RATE;
    ...
    >
      ...
      <div className="relative overflow-hidden aspect-square bg-muted">
        ...
        <div className="absolute top-2 right-2">
          <Badge
            variant="outline"
            className="text-[10px] py-0.5 px-1.5 bg-background/80 backdrop-blur-sm flex items-center gap-1"
          >
